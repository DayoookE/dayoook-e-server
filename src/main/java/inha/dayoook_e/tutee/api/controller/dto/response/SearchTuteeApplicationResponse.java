...
public record SearchTuteeApplicationResponse(
        @NotNull
        @Schema(description = "신청 그룹 id", example = "1")
        Integer id,

        @NotNull @Schema(description = "튜터 정보")
        TutorInfoResponse tutorInfo,

        @NotEmpty @Schema(description = "튜터 사용 가능 언어")
        List<SearchLanguagesResponse> languages,

        @NotEmpty @Schema(description = "신청 시간 목록")
        List<ScheduleTimeSlot> scheduleTimeSlots,

        @NotNull @Schema(description = "신청 시간", example = "2024-08-01T00:00:00")
        LocalDateTime createdAt,

        @NotNull @Schema(description = "신청 상태", example = "APPLYING")
        Status status,

        @NotNull @Schema(description = "메시지", example = "안녕하세요")
        String message
) {
}
